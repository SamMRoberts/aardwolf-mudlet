...
local function defaultTree()
  return {
    ...
    first = {
      ...
      ratio = 0.53,
      first = {type = "stack", tabs = {"aardwolf-vibe.buffs-window"},
        active = "aardwolf-vibe.buffs-window"},
      second = {type = "stack", tabs = {"aardwolf-vibe.ascii-map"},
...
function Workspace.new(api, settings)
  ...
  local function readState()
    ...
    if not file then
      if api.lfs.attributes(path) == nil then
        -- Earlier installs already have native dock placements to preserve.
        if api.AardwolfVibeCommandQueueLayout ~= nil
            or api.AardwolfVibeSpellupsWindowLayout ~= nil
            or (settings.path and api.lfs.attributes(settings.path) ~= nil) then
          state.enabled = false
        end
        return true
      end
  ...
  function self:registerPanel(spec)
    ...
    if (added or (source == "defaults" and state.enabled)) and not locked then
      ok, message = writeState()
      if not ok then
        if record.root then pcall(record.spec.unmount, record.root) end
        deleteWidget(record.standalone)
  ...
  function self:reset()
    ...
    local target = firstStack(nextState.tree)
