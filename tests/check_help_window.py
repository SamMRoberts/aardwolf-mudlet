...
class HelpWindowTests(unittest.TestCase):
    def runtime(self, start=True):
        ...
        lua.execute("help=factory.new(_G)")
        if start:
    ...
    def test_window_defaults_transient_commands_and_tag_requests(self):
        ...
        lua.execute(r'''
          ...
          assert(window.cons.restoreLayout and window.cons.autoDock and not window.cons.docked)
          ...
          assert(#sent==2 and help:status().tagRequests==2 and not help:status().captureActive)
    ...
    def test_snapshot_render_and_tag_request_failures_are_recoverable(self):
        ...
        lua.execute(r'''
          ...
          assert(help:status().tagState=="request-failed")
          fail.send=nil;assert(help:requestTags("connection"))
          assert(help:status().tagState=="requested")
        ''')
