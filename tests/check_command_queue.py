...
class CommandQueueTests(unittest.TestCase):
    ...
    def test_exact_echo_removes_oldest_identical_and_hides_server_echoes(self):
        ...
        lua.execute('''
          ...
          assert(queue:status().pending==4)
          assert(#visible==1 and visible[1]=="Not You entered: look")
          assert(deletedLines==2)
          incoming("You entered: look")
          assert(queueWindow().text=="1. north\\n2. look\\n3. east\\n")
          incoming("You entered: look")
          assert(queueWindow().text=="1. east\\n")
          incoming("You entered: north")
          assert(queueWindow().text=="1. east\\n")
          incoming("You entered: east")
          assert(queueWindow().text=="No commands queued\\n")
          assert(#visible==1 and deletedLines==6)
        ''')

    def test_later_echo_clears_all_earlier_commands(self):
        lua = self.runtime()
        lua.execute('''
          publishStatus(3)
          fire("sysDataSendRequest", "stale")
          fire("sysDataSendRequest", "north")
          fire("sysDataSendRequest", "look")
          fire("sysDataSendRequest", "east")
          incoming("You entered: look")
          assert(queueWindow().text=="1. east\\n")
          fire("sysDataSendRequest", "stale again")
          fire("sysDataSendRequest", " c 'cure p")
          fire("sysDataSendRequest", "south")
          incoming("You entered: c 'cure p")
          assert(queueWindow().text=="1. south\\n")
          assert(queue:status().pending==1 and #visible==0 and deletedLines==2)
          assert(#sent==1 and sent[1].command=="config echocommands on")
        ''')
