...
class PlaceholderTests(unittest.TestCase):
    ...
    def test_fresh_reported_exits_replace_manual_stubs(self):
        self.check('''
          ...
          assert(not r.stubs.east and r.exits.east==103 and localID(103)==103)
          packet(101,{n=102,w=-1}); setExitStub(localID(101),'w',false)
          packet(101,{n=102,w=104})
          assert(not r.stubs.west and r.exits.west==104 and localID(104)==104)
    ...
    def test_unknown_destination_clears_old_owned_link_without_recreating_it(self):
        self.check('''
          ...
          packet(101,{e=-1}); assert(not rooms[from].exits.east)
        ''')
    ...
    def test_confirmed_position_collision_retains_provisional_position(self):
        self.check('''
          ...
          assert(rooms[id].x==50 and rooms[id].y==-50 and rooms[id].name~='')
          assert(mapper.conflicts==0)
        ''')

    def test_gmcp_promotes_placeholder_but_preserves_symbol_and_notes(self):
        self.check('''
          ...
          assert(r.name=='Terrain 102' and r.symbol=='!' and r.x==77 and getRoomEnv(id)>=1000)
          assert(r.data.note=='keep' and r.data['AardwolfToolbox:terrain']=='forest')
          assert(mapper.promoted==1 and mapper.conflicts==0)
          terrainPacket(102,'water'); assert(environmentColors[getRoomEnv(id)][3]==220)
        ''')

    def test_gmcp_zone_and_continent_position_replace_provisional_area(self):
        self.check('''
          packet(101,{e=102}); local id=localID(102)
          local area=addAreaName('my area'); setRoomArea(id,area)
          packet(102,{},'world',{cont=1,id=0,x=50,y=50})
          assert(rooms[id].area==areas.world and rooms[id].x==50 and rooms[id].y==-50)
          assert(mapper.promoted==1 and mapper.conflicts==0)
        ''')
    ...
    def test_failures_do_not_duplicate_partial_rooms_or_claim_stub_success(self):
        self.check('''
          ...
          mapper.start(); local stubs=mapper.stubs; failNext('setExitStub'); packet(101,{w=-1})
          assert(not mapper.enabled and mapper.stubs==stubs)
        ''')
    ...
    def test_legacy_rooms_remain_visited_and_edits_survive(self):
        self.check('''
          ...
          assert(r.name=='Room 101' and r.symbol=='!' and mapper.promoted==0)
          assert(mapper.placeholders==1)
        ''')

    def test_reported_zone_replaces_placeholder_area_during_promotion(self):
        self.check('''
          packet(101,{e=102},'academy'); local id=localID(102)
          local area=rooms[id].area; setAreaName(area,'My academy')
          packet(102,{},'academy')
          assert(rooms[id].area==areas.academy and areas['My academy']==area and areas.academy~=area)
          assert(mapper.promoted==1 and mapper.conflicts==0)
        ''')
