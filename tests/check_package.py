    ...
    def setUp(self):
        with zipfile.ZipFile(ROOT / "build/AardwolfToolbox.mpackage") as archive:
            self.assertEqual(set(archive.namelist()), {
                ...
                "console-cleanup.lua", "mobs.lua", "mob-state.lua", "mob-protocol.lua", "mob-pane.lua", "abilities.lua", "ability-capture.lua", "ability-store.lua", "ability-model.lua", "ability-fields.lua", "ability-picker.lua", "query-coordinator.lua",
            ...
            self.extra = {name: archive.read(name+".lua").decode() for name in ("console-cleanup","mobs","mob-state","mob-protocol","mob-pane","abilities","ability-capture","ability-store","ability-model","ability-fields","ability-picker","query-coordinator","incoming","borders","ascii-map","settings-window","consider","gmcp-cache","player-panel","help-pane","inventory","utility-bar","appearance","dashboard-data","dashboard","spells","spellup","action-bar","navigation","shortcuts")}
