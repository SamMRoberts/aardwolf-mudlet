...
class PackageTests(unittest.TestCase):
    def setUp(self):
        with zipfile.ZipFile(ROOT / "build/AardwolfToolbox.mpackage") as archive:
            self.assertEqual(set(archive.namelist()), {
                "AardwolfToolbox.xml", "config.lua", "chat-model.lua", "chat.lua", "chat-workspace.lua", "chat-bell.wav", "chat-chime.wav", "chat-pop.wav", "automapper.lua", "mapper-identity.lua", "map-travel.lua",
                ...
                "objective-state.lua", "objective-store.lua", "objective-protocol.lua", "objective-tracker.lua", "menu-keys.lua", "quest-history.lua", "progression-history.lua", "history-store.lua", "history-pane.lua", "notifications.lua", "notification-pane.lua", "preferences-files.lua", "map-workspace.lua", "map-workspace-pane.lua", "item-actions.lua", "workspace-browser.lua", "launcher.lua", "chat-search.lua", "sidebar-shell.lua", "console-text.lua", "components.lua", "readiness.lua", "configuration.lua", "settings-window.lua", "vitals.lua", "tags.lua", "incoming.lua", "borders.lua", "ascii-map.lua", "consider.lua", "gmcp-cache.lua", "player-panel.lua", "help-pane.lua", "inventory.lua", "item-state.lua", "utility-bar.lua", "appearance.lua", "dashboard-data.lua", "dashboard.lua", "view-hosts.lua", "dashboard-panels.lua", "spells.lua", "spellup.lua", "action-bar.lua", "navigation.lua", "shortcuts.lua",
            })
            self.extra = {name: archive.read(name+".lua").decode() for name in ("chat-model","chat","chat-workspace","objective-state","objective-store","objective-protocol","objective-tracker","menu-keys","quest-history","progression-history","history-store","history-pane","notifications","notification-pane","preferences-files","map-workspace","map-workspace-pane","item-actions","workspace-browser","launcher","chat-search","sidebar-shell","console-text","components","readiness","map-travel","mapper-identity","console-cleanup","mobs","mob-state","mob-protocol","mob-pane","mob-actions","abilities","ability-capture","ability-store","ability-model","ability-fields","ability-picker","query-coordinator","incoming","borders","ascii-map","settings-window","consider","gmcp-cache","player-panel","help-pane","item-state","inventory","utility-bar","appearance","dashboard-data","dashboard","view-hosts","dashboard-panels","spells","spellup","action-bar","navigation","shortcuts")}
    ...
    def test_map_double_click_setting_and_shutdown_restore_hook(self):
        self.lua.execute('''
          originalWalk=function() end;doSpeedWalk=originalWalk
          AardwolfToolbox.start()
          local c=AardwolfToolbox.config
          assert(AardwolfToolbox.mapTravel.enabled and doSpeedWalk~=originalWalk)
          assert(c.set('mapper','double_click_run',false))
          assert(not AardwolfToolbox.mapTravel.enabled and doSpeedWalk==originalWalk)
          assert(c.set('mapper','enabled',false))
          assert(c.set('mapper','double_click_run',true))
          assert(AardwolfToolbox.mapTravel.enabled and not AardwolfToolbox.mapper.enabled)
          AardwolfToolbox.stop();assert(doSpeedWalk==originalWalk)
        ''')
