...
local items={
  ...
  {id='43',name='@Ra sturdy helmet',level=55,type=9,flags='',location='equipped',wear=4,timer=-1,fresh=true},
}
local abilities={}
...
replace(t.inventory,'list',function(scope)
  local rows={};for _,r in ipairs(items) do if scope==r.location then rows[#rows+1]=copy(r) end end;return rows
end)
replace(t.inventory,'get',function(id) for _,r in ipairs(items) do if r.id==id then return copy(r) end end end)
replace(t.inventory,'status',function() return {enabled=true,fresh={carried=true,equipped=true},revision=1} end)
