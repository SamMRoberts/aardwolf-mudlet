...
class WorkspaceTests(unittest.TestCase):
    ...
    def test_default_tree_validation_moves_ratios_visibility_and_bounds(self):
        ...
        lua.execute(r'''
          ...
          assert(valid and not message and valid.enabled==false and valid.visible==true)
          assert(valid.tree.type=="split" and valid.tree.orientation=="vertical")
          assert(valid.tree.ratio==0.45 and valid.tree.first.active=="aardwolf-vibe.ascii-map")
          assert(valid.tree.second.active=="aardwolf-vibe.chat")
          local moved=assert(Factory.movePanel(valid,"aardwolf-vibe.chat","root.first","center",1))
          assert(moved.tree.first.tabs[1]=="aardwolf-vibe.chat" and moved.tree.first.active=="aardwolf-vibe.chat")
          assert(countPanel(moved.tree,"aardwolf-vibe.chat")==1)
          local split=assert(Factory.movePanel(moved,"aardwolf-vibe.buffs-window","root.first","left"))
          assert(split.tree.first.type=="split" and split.tree.first.orientation=="horizontal")
          local clamped=assert(Factory.setRatio(split,"root.first",99))
          ...
          duplicate.tree.second.tabs[#duplicate.tree.second.tabs+1]="aardwolf-vibe.ascii-map"
