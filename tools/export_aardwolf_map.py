#!/usr/bin/env python3
"""Export the Aardwolf MUSHclient mapper snapshot as a deterministic Mudlet map asset.

The database is deliberately opened read-only.  This converter is also the schema and
referential-integrity gate for the redistributed JSON resource.
"""
...
class ExportError(ValueError):
    """A source snapshot is malformed or incompatible with the v11 exporter."""
...
def load_snapshot(connection: sqlite3.Connection) -> tuple[dict[str, Any], dict[str, Any]]:
    area_rows = [
    ...
    areas_by_uid = {area["uid"]: area for area in area_rows}
    environments_by_name = {environment["name"]: environment for environment in environment_rows}
    if len(areas_by_uid) != len(area_rows):
        raise ExportError("duplicate area uid")
    if len(environments_by_name) != len(environment_rows):
    ...
    for row in connection.execute(
        ...
        rooms.append(
            {
                "vnum": vnum,
                "name": nullable_text(row[1]) or "",
                "area_uid": area_uid,
                "building": nullable_text(row[3]),
                "terrain": terrain,
                "terrain_uid": environments_by_name[terrain]["uid"],
                "info": nullable_text(row[5]),
                "notes": nullable_text(row[6]),
                "source_coordinates": [nullable_int(row[7]), nullable_int(row[8]), nullable_int(row[9])],
                "norecall": int(row[10] or 0),
                "noportal": int(row[11] or 0),
    ...
    for row in connection.execute("SELECT dir, fromuid, touid, level FROM exits"):
        ...
        source = require_integer_uid(row[1], "exit source")
        destination = require_integer_uid(row[2], "exit destination")
    ...
    exits.sort(key=lambda exit_row: (exit_row["from_vnum"], DIRECTION_INDEX[exit_row["direction"]], exit_row["to_vnum"], exit_row["level"]))

    referenced_environment_uids = {room["terrain_uid"] for room in rooms}
    metadata = {
        "areas": area_rows,
        "environments": [environment for environment in environment_rows if environment["uid"] in referenced_environment_uids],
        "rooms": rooms,
        "exits": exits,
...
def generate_layout(rooms: list[dict[str, Any]], exits: list[dict[str, Any]]) -> dict[str, int]:
    """Assign deterministic local positions; constraints never cause an exit to be discarded."""
    rooms_by_vnum = {room["vnum"]: room for room in rooms}
    ...
    for edge_list in outbound.values():
        edge_list.sort(key=lambda exit_row: (DIRECTION_INDEX[exit_row["direction"]], exit_row["to_vnum"]))
...
def build_export(source: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    map_data = {
        ...
        "source": {"file_name": source.name, "sha256": source_hash, "sqlite_user_version": DATABASE_USER_VERSION},
        "areas": snapshot["areas"],
        "environments": snapshot["environments"],
        "import_area_uids": snapshot["import_area_uids"],
        "rooms": snapshot["rooms"],
        "exits": snapshot["exits"],
        "layout": layout,
        "counts": {
            "rooms": len(snapshot["rooms"]),
            "standard_exits": len(snapshot["exits"]),
            "populated_areas": len(snapshot["import_area_uids"]),
            "source_areas": len(snapshot["areas"]),
    ...
    report = {
        ...
        "source_schema": {
            "sqlite_user_version": DATABASE_USER_VERSION,
            "required_tables": sorted(REQUIRED_COLUMNS),
        },
        "source_table_counts": table_counts,
        "export_counts": map_data["counts"],
        "reference_checks": integrity,
        "layout": layout,
...
def main(arguments: list[str]) -> int:
    ...
    try:
        map_data, report = build_export(arguments.input)
        write_atomic(arguments.output, stable_json(map_data))
        write_atomic(arguments.report_json, json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n")
        write_atomic(arguments.report_markdown, markdown_report(report))
