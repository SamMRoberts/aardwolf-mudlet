...
When mapper-conversion inputs are present, also read
[Aardwolf mapper conversion review](references/aardwolf-mapper-conversion-review.md) before the
mapper branch.
...
3. Detect mapper-conversion scope only when the inputs include an Aardwolf map resource
   (`aardwolf-map-v11.json`), mapper inventory report, or source Aardwolf SQLite database. If no
   such input exists, skip this branch. Otherwise, require the source database and canonical map
   resource as review evidence; their absence makes the conversion unreviewable and blocks
   approval. From this skill directory run
   `python3 ../../scripts/validate_aardwolf_map_conversion.py --input DATABASE --project PROJECT`
   and retain its output. Add `--artifacts ARTIFACTS` when the conversion reports/artifacts
   directory is available. Treat success as structural evidence only: reconcile its source hash,
   ordered records, counts, references, resource inclusion, and report values with the examined
   source and package. Review the mapper behavior using the mapper reference: ownership markers
   and stable hashes must distinguish package rooms/areas from foreign map data; no code may
   clear, load, replace, recolor, or otherwise destructively rewrite the profile map; environment
   allocation and room collisions must preserve foreign ownership; same-snapshot imports must be
   idempotent, changed snapshots must refresh only package-owned source data, and cancellation or
   reload must resume safely. Require bounded import batches, named lifecycle records with complete
   cleanup, and read-only `gmcp.room.info` centering. Flag untracked registrations, destructive
   mapper APIs, guessed GMCP data, or any path that can overwrite foreign rooms, exits, colors, or
   user map data.
4. Inspect the source contract: `src/scripts`, `aliases`, `triggers`, `timers`, `keys`, and
...
5. Review protocol and runtime behavior using the protocol reference. Prefer Aardwolf GMCP data
...
6. Trace load, enable, connect, disconnect, reload, disable, and unload paths. Verify ownership
...
7. Review persistence, UI, portability, and security: version and validate saved data; preserve
...
8. Inspect aliases, triggers, timers, scripts, resources, docs, and tests against observable
...
9. Compare the native XML/`.mpackage` and Muddler representations. Inspect existing artifacts or,
...
10. When the project is large enough that at least two stable review branches are independent and
...
11. Produce a review report with package identity, inputs examined, validation commands and
