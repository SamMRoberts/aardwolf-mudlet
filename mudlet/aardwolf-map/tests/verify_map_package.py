...
def main() -> int:
    ...
    if "--artifacts" in sys.argv:
        ...
        with zipfile.ZipFile(PACKAGE / "dist" / "aardwolf-map.mpackage") as archive:
            ...
            require("version = [[1.1.0]]" in archive.read("config.lua").decode("utf-8"), "distribution package version is stale")
    ...
    require([alias["name"] for alias in aliases] == ["aardwolf_map.import", "aardwolf_map.import_cancel", "aardwolf_map.status", "aardwolf_map.palette"], "explicit map aliases are missing")
    ...
    require("setRoomEnv(room_id, environment_id)" in lifecycle and "environment_id_for(room.terrain_uid)" in lifecycle, "owned rooms do not receive allocated terrain environments")
    require("registerNamedTimer" in lifecycle and "deleteNamedTimer" in lifecycle, "import batching cannot be cancelled")
    require("aardwolf-map::event::room-info" in lifecycle and "gmcp.room.info" in lifecycle and "sysUninstallPackage" in lifecycle and "sysExitEvent" in lifecycle, "named lifecycle handlers are missing")
    require("function aardwolf_map.integration.snapshot()" in lifecycle and "aardwolf-map::status-changed" in lifecycle, "map integration status API is missing")
    require("is_reserved_environment_id" in lifecycle and "getCustomEnvColor" in lifecycle and "getRoomEnv" in lifecycle, "collision-safe environment allocation is missing")
