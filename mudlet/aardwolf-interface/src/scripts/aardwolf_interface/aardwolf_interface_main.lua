...
local DETAILS_ACK_TIMER = "aardwolf-interface::timer::details-spellup-ack"
...
function aardwolf_interface.details.remove_spellup_ack()
  local runtime = aardwolf_interface.details.runtime
  if runtime.ack_trigger_id and type(killTrigger) == "function" then
    pcall(killTrigger, runtime.ack_trigger_id)
  end
  runtime.ack_trigger_id = nil
  delete_named_timer(DETAILS_ACK_TIMER)
end

function aardwolf_interface.details.on_spellup_ack()
  if type(deleteLine) == "function" then
    deleteLine()
  end
  aardwolf_interface.details.remove_spellup_ack()
end

function aardwolf_interface.details.suppress_spellup_ack()
  aardwolf_interface.details.remove_spellup_ack()
  local runtime = aardwolf_interface.details.runtime
  if type(tempRegexTrigger) == "function" then
    runtime.ack_trigger_id = tempRegexTrigger("(?i)^[^\\n]*spellup[^\\n]*(?:on|off|enabled|disabled)[^\\n]*$", aardwolf_interface.details.on_spellup_ack)
  end
  if runtime.ack_trigger_id and type(registerNamedTimer) == "function" then
    registerNamedTimer(TIMER_USER, DETAILS_ACK_TIMER, 3.0, aardwolf_interface.details.remove_spellup_ack, true)
  end
end
...
local function commit_capture(capture)
  ...
  elseif capture.kind == "tags" and capture.spellup_prior ~= nil then
    ...
    if capture.spellup_prior == false and aardwolf_interface.settings.details_are_visible() and type(send) == "function" then
      aardwolf_interface.details.suppress_spellup_ack()
...
function aardwolf_interface.details.capture_line(raw_line)
  ...
  elseif capture.kind == "resists" then
    ...
    local lowered = strip_aardwolf_colors(line):lower()
    if lowered:find("resist", 1, true) then
      capture.seen_header = true
      return true
    end
    if line:match("^%s*[%-%=|+]+%s*$") then
      return true
    end
    if not capture.seen_header and payload == nil then
      return false
    end
    ...
    return payload ~= nil or line:match("^%s*$") ~= nil
  elseif capture.kind == "tags" then
    ...
    if lowered:find("tag", 1, true) or line:match("^%s*[%-%=|+]+%s*$") then
      return true
    end
    if line:match("^%s*[%w_%- ]+%s*[:=]%s*[%a]+%s*$") then
      return true
    end
    return false
  end
...
function aardwolf_interface.lifecycle.unregister()
  ...
  if type(deleteNamedTimer) == "function" then
    ...
    deleteNamedTimer(TIMER_USER, START_TIMER)
    deleteNamedTimer(TIMER_USER, TICK_TIMER)
    deleteNamedTimer(TIMER_USER, DETAILS_ACK_TIMER)
  end
end
