...
function aardwolf_interface.settings.validate(candidate)
  ...
  local claim = candidate.border_claim
  if type(claim) == "table" then
    local base = bounded_number(claim.base, 0, 10000)
    local width = bounded_number(claim.width, 0, 2000)
    local applied = bounded_number(claim.applied, 0, 12000)
    if base and width and applied and applied == base + width then
      validated.border_claim = {base = base, width = width, applied = applied}
    end
  end
  return validated
end
...
-- UI owns the sidebar, all widgets, the right-border claim, and mapper display.
...
function aardwolf_interface.ui.build()
  ...
  widgets.condition = new_label("condition", primary)
  ...
  for _, name in ipairs({"room", "stats", "condition", "group", "map_status", "details_header", "details_equipment", "details_bags"}) do
...
function aardwolf_interface.ui.apply_theme()
  ...
  for _, name in ipairs({"room", "stats", "condition", "group", "map_status", "details_equipment", "details_bags"}) do
...
function aardwolf_interface.ui.reflow()
  ...
  local gauge_y = room_y + 56
  for index, name in ipairs({"hp", "mana", "moves", "tnl", "enemy"}) do
    local item_y = gauge_y + (index - 1) * 28
    safe_call(widget(name), "move", margin, item_y)
    safe_call(widget(name), "resize", content_width, 24)
  end
  local stats_y = gauge_y + 5 * 28
  safe_call(widget("stats"), "move", margin, stats_y)
  safe_call(widget("stats"), "resize", content_width, 108)
  local condition_y = stats_y + 116
  safe_call(widget("condition"), "move", margin, condition_y)
  safe_call(widget("condition"), "resize", content_width, 70)
  local group_y = condition_y + 78
...
<tr><td colspan="2"><b>Trains</b> %s</td><td colspan="2"><b>Practices</b> %s</td></tr>
  ...
  local condition_text = string.format([[
<table width="100%%" cellspacing="0" cellpadding="1">
<tr><td colspan="4"><b>Condition</b></td></tr>
<tr><td><b>Hunger</b> %s%%</td><td><b>Thirst</b> %s%%</td><td><b>Position</b> %s</td><td><b>State</b> %s</td></tr>
</table>]],
    display_number(status.hunger), display_number(status.thirst),
    html_escape(status.pos or "--"), html_escape(CHARACTER_STATES[state_number] or display_number(state_number)))
  safe_call(widget("condition"), "echo", condition_text)
