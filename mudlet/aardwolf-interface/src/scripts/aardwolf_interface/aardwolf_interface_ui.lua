...
local TABS = {"overview", "character", "group", "inventory"}
local TAB_LABELS = {overview = "Overview", character = "Character", group = "Group", inventory = "Inventory"}
...
local function action_module()
  return aardwolf_interface and aardwolf_interface.actions
end

local function execute_action(id)
  return function()
    local handlers = action_module()
    if handlers and type(handlers.execute) == "function" then
      local ok, failure = handlers.execute(id)
      if not ok and failure then ui.message(failure) end
      ui.request_render()
      return
    end
    ui.message("Action system is not available yet.")
  end
end
...
local function pending_action()
  ...
  if type(handlers.pending) == "function" then return handlers.pending() end
  return type(handlers.pending) == "table" and handlers.pending or nil
end

local function confirm_action()
  local handlers = action_module()
  if not handlers then return end
  if handlers.confirm_pending then handlers.confirm_pending()
  elseif handlers.confirm then handlers.confirm() end
  ui.request_render()
end
...
local function cancel_action()
  local handlers = action_module()
  if not handlers then return end
  if handlers.cancel_pending then handlers.cancel_pending()
  elseif handlers.cancel then handlers.cancel() end
  ui.request_render()
end
...
local function submit_custom_editor()
  ...
  if ui.editing_action_id then
    local handlers = action_module()
    local update = handlers and (handlers.update_custom or handlers.update)
    if update then ok, error_message = update(ui.editing_action_id, label, command, category) end
  else
    local handlers = action_module()
    local add = handlers and (handlers.add_custom or handlers.add)
...
function ui.apply_theme()
  ...
  for _, name in ipairs({"map_status", "map_legend", "overview_card", "character_card", "group_card", "inventory_card", "drawer_actions", "editor_help", "confirm_text"}) do safe(widget(name), "setStyleSheet", section) end
...
function ui.reflow()
  ...
  elseif l.collapsed then
    ...
    local tab_width = (dock_width - CONTENT_MARGIN * 2 - GAP * 3) / 4
...
function ui.render()
  ...
  render_header(); render_context(); render_map(); render_overview(); render_character(); render_group(); render_inventory(); render_dock_header(); render_drawer(); render_overlays()
  local active = data().active_tab or "overview"
...
function ui.set_tab(tab)
  ...
  local valid = {overview = true, character = true, group = true, inventory = true}
  ...
  if current_uses_details and tab ~= data().active_tab and aardwolf_interface.details and aardwolf_interface.details.stop then aardwolf_interface.details.stop(true) end
